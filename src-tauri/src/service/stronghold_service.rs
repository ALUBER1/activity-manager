...
use crate::{
    repository::storage_repository::StorageRepository,
    service::storage_service::{get_storage, store_storage},
};
...
pub fn verify(
    ...
    let parsed = match PasswordHash::new(&binding) {
        ...
        Err(_) => {
            println!("problem");
            return StorageEntry::default();
        }
    };
    println!(
        "is ok: {}",
        argon2
            .verify_password(storage_entry.value.as_bytes(), &parsed)
            .is_ok()
            .to_string()
    );
    StorageEntry::new(
...
pub fn store_password(
    ...
    store_storage(
        app,
        StorageEntry::new(storage_entry.key.clone(), encrypt(storage_entry.clone())),
        state,
    );
    storage_entry
}
